...
def encrypt (key : PublicKey) (m : ℕ) :=
  ∑ k ∈ Finset.Ico 0 (key.n.log m + 1), encryptBlock key (key.n.digit m k) * key.n ^ k

def decrypt (key : PrivateKey) (c : ℕ) :=
  ∑ k ∈ Finset.Ico 0 (key.n.log c + 1), decryptBlock key (key.n.digit c k) * key.n ^ k
...
theorem decrypt_encrypt (keys : KeyPair) (m : ℕ) :
  ...
  have hb1 : 2 ≤ keys.n := by
    ...
    nlinarith
  by_cases hm : m = 0
  · simp [encrypt, decrypt, hm, Nat.digit]
  unfold decrypt
    ...
    · unfold encrypt
      calc
        ∑ k ∈ Finset.Ico 0 (Nat.log keys.n m + 1),
          encryptBlock keys.toPublicKey (keys.n.digit m k) * keys.n ^ k =
        (∑ k ∈ Finset.Ico 0 (Nat.log keys.n m),
          encryptBlock keys.toPublicKey (keys.n.digit m k) * keys.n ^ k +
            encryptBlock keys.toPublicKey (keys.n.digit m (Nat.log keys.n m)) *
        ...
        _ ≥ 1 * keys.n ^ (Nat.log keys.n m) := by
          gcongr
          contrapose! hm
          replace h1 : encryptBlock keys.toPublicKey (keys.n.digit m (Nat.log keys.n m)) = 0
            := by omega
          rw [encryptBlock_eq_zero_iff keys (keys.n.digit m (Nat.log keys.n m))
                (by apply keys.n.digit_lt; omega)] at h1
          simpa [Nat.digit_log_eq_zero_iff _ _ hb1] using h1
        _ = _ := by ring
    · unfold encrypt
      ...
      apply Nat.mod_lt
      omega
  have th2 k : keys.n.digit (encrypt keys.toPublicKey m) k =
      encryptBlock keys.toPublicKey (keys.n.digit m k) := by
    unfold encrypt
    dsimp [KeyPair.toPublicKey]
    rw [Nat.digit_sum_mul_base_pow]
    · split_ifs with hk
      · rfl
      · symm
        calc
  ...
  calc
    ∑ k ∈ Finset.Ico 0 (Nat.log keys.n (encrypt keys.toPublicKey m) + 1),
      decryptBlock keys.toPrivateKey
        (keys.n.digit (encrypt keys.toPublicKey m) k) * keys.n ^ k =
    ∑ k ∈ Finset.Ico 0 (Nat.log keys.n (encrypt keys.toPublicKey m) + 1),
      keys.n.digit m k * keys.n ^ k := by
      apply Finset.sum_congr rfl
      intro i hi
      congr 2
      rw [th2]
      rw [decryptBlock_encryptBlock]
      apply keys.n.digit_lt
      omega
    _ = m := by simpa [th1] using Nat.sum_digit_mul_base_pow hb1 m
